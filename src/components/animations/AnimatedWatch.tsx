...
function Watch() {
  ...
  useFrame(() => {
    ...
    // Second hand ticks in discrete steps for a mechanical feel.
      ...
      <group>
        <Center>
          ...
          <mesh position={[0, 0, 0.21]} rotation={faceCamera}>
            ...
            <meshStandardMaterial color="#1a1a2e" metalness={0.35} roughness={0.55} />
          </mesh>

          {/* Roman numeral hour markers (I–XII) */}
          {ROMAN_NUMERALS.map((numeral, i) => {
            // i = 0 sits at the top (12 o'clock), then clockwise.
...
export function AnimatedWatch({ className }: AnimatedWatchProps) {
  return (
    ...
    >
      ...
      <motion.div
        aria-hidden
        className="absolute left-1/2 top-1/2 -z-10 h-3/4 w-3/4 -translate-x-1/2 -translate-y-1/2 rounded-full bg-gold/20 blur-[100px]"
      ...
      <Canvas camera={{ position: [0, 0.4, 6.8], fov: 42 }} dpr={[1, 2]}>
        ...
        <spotLight position={[5, 6, 5]} angle={0.4} penumbra={1} intensity={1.2} />
        <spotLight position={[-5, -3, 5]} angle={0.4} penumbra={1} intensity={0.5} />
