        ...
        <div className="grid gap-6 md:grid-cols-3">
          {collections.map((collection, i) => (
            <FadeInSection key={collection.id} direction="up" delay={i * 0.15}>
              ...
              >
                <div className="relative aspect-[3/4] overflow-hidden bg-darkGray">
                  <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/30 to-transparent" />
                  <div className="absolute inset-0 flex flex-col justify-end p-8">
                    <p className="mb-2 text-xs font-medium tracking-[0.2em] uppercase text-gold">
                      Since {collection.yearIntroduced}
                    </p>
                    <h3 className="mb-2 text-2xl font-light text-cream">
                      {collection.name}
                    </h3>
                    <p className="mb-4 text-sm leading-relaxed text-cream/60">
                      {collection.tagline}
                    </p>
                    <div className="flex items-center gap-2 text-xs font-medium tracking-wider uppercase text-gold opacity-0 transition-opacity group-hover:opacity-100">
